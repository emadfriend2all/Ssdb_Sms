    ...
    private static IEnumerable<FshTenantInfo> TenantStoreSetup(IApplicationBuilder app)
    {
        ...
        if (tenantDbContext.TenantInfo.Find(TenantConstants.Root.Id) is null)
        {
            var rootTenant = new FshTenantInfo(
                ...
                SubscriptionTypes.Free.ToString(),
                TenantConstants.Root.Address,
                TenantConstants.Root.PhoneNumber,
                TenantConstants.Root.Logo,
                TenantConstants.Root.PrimaryColor,
                TenantConstants.Root.SecondaryColor,
                TenantConstants.Root.ReportHeader,
                TenantConstants.Root.ReportFooter
                );

            rootTenant.Organization = new FshTenantOrganization
            {
                CommonName = TenantConstants.DefaultTenantOrganization.CommonName,
                SerialNumber = TenantConstants.DefaultTenantOrganization.SerialNumber,
                OrganizationIdentifier = TenantConstants.DefaultTenantOrganization.OrganizationIdentifier,
                IdentifierType = TenantConstants.DefaultTenantOrganization.IdentifierType,
                OrganizationUnitName = TenantConstants.DefaultTenantOrganization.OrganizationUnitName,
                OrganizationName = TenantConstants.DefaultTenantOrganization.OrganizationName,
                CountryName = TenantConstants.DefaultTenantOrganization.CountryName,
                InvoiceType = TenantConstants.DefaultTenantOrganization.InvoiceType,
                LocationAddress = TenantConstants.DefaultTenantOrganization.LocationAddress,
                IndustryBusinessCategory = TenantConstants.DefaultTenantOrganization.IndustryBusinessCategory,
                TenantId = TenantConstants.DefaultTenantOrganization.TenantId
            };
