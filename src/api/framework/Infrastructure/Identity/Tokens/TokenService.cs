...
public sealed class TokenService : ITokenService
{
    ...
    private async Task<List<Claim>> GetClaimsAsync(FshUser user, string ipAddress)
    {
        var tenentInfo = await _tenantService.GetBasicTenantInfoAsync(_multiTenantContextAccessor!.MultiTenantContext.TenantInfo!.Id);
        string tenantInfoJson = JsonConvert.SerializeObject(tenentInfo) ?? string.Empty;
