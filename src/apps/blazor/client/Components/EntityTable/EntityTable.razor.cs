    ...
    where TRequest : new()
{
    ...
    [Inject]
    protected IStringLocalizer<SharedResource> _localizer { get; set; } = default!;
    ...
    private async Task<bool> CanDoActionAsync(string? action, AuthenticationState state)
    {
        ...
        return bool.TryParse(action, out bool isTrue)
            ? isTrue
    ...
    private async Task InvokeModal(TEntity? entity = default)
    {
        ...
        if (isCreate)
        {
            ...
            title = string.Format(_localizer["Create {0}"], Context.EntityName);
            successMessage = string.Format(_localizer["{0} Created"], Context.EntityName);
        }
        else
        {
            ...
            title = string.Format(_localizer["Edit {0}"], Context.EntityName);
            successMessage = string.Format(_localizer["{0} Updated"], Context.EntityName);
        }
    ...
    private async Task Delete(TEntity entity)
    {
        ...
        string deleteContent = _localizer["You're sure you want to delete {0} with id '{1}'?"];
        var parameters = new DialogParameters
        {
